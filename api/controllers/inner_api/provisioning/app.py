"""Provisioning inner API for app lifecycle management.

Provides CRUD endpoints for Dify applications, driven by DSL YAML content.
Intended for programmatic consumers (Terraform, CI/CD) and authenticated via
``X-Inner-Api-Key`` (``enterprise_inner_api_only``).

All endpoints are workspace-scoped: the caller must supply the target
``workspace_id`` in the URL path.  Workspace membership is assumed to have
been validated by the outer control plane (e.g. the Go admin-api service).
"""
...
register_schema_models(inner_api_ns, ProvisioningAppCreatePayload, ProvisioningAppUpdatePayload, ProvisioningAppListQuery)
...
@inner_api_ns.route("/provisioning/workspaces/<string:workspace_id>/apps")
class ProvisioningAppListApi(Resource):
    """List apps in a workspace."""

    @setup_required
    @enterprise_inner_api_only
    @inner_api_ns.doc("provisioning_list_apps")
    @inner_api_ns.doc(description="List apps in a workspace")
    @inner_api_ns.expect(inner_api_ns.models[ProvisioningAppListQuery.__name__])
    ...
    @setup_required
    @enterprise_inner_api_only
    @inner_api_ns.doc("provisioning_create_app")
    @inner_api_ns.doc(description="Create an app from DSL YAML content")
    @inner_api_ns.expect(inner_api_ns.models[ProvisioningAppCreatePayload.__name__])
    @inner_api_ns.doc(
        responses={
    ...
    def post(self, workspace_id: str):
        args = ProvisioningAppCreatePayload.model_validate(inner_api_ns.payload or {})
...
@inner_api_ns.route("/provisioning/workspaces/<string:workspace_id>/apps/<string:app_id>")
class ProvisioningAppApi(Resource):
    """Get, update, or delete a specific app."""

    @setup_required
    @enterprise_inner_api_only
    @inner_api_ns.doc("provisioning_get_app")
    @inner_api_ns.doc(description="Get app detail with DSL export")
    @inner_api_ns.doc(
        responses={
    ...
    @setup_required
    @enterprise_inner_api_only
    @inner_api_ns.doc("provisioning_update_app")
    @inner_api_ns.doc(description="Update an app by re-importing DSL YAML content")
    @inner_api_ns.expect(inner_api_ns.models[ProvisioningAppUpdatePayload.__name__])
    @inner_api_ns.doc(
        responses={
    ...
    def put(self, workspace_id: str, app_id: str):
        ...
        args = ProvisioningAppUpdatePayload.model_validate(inner_api_ns.payload or {})

        # Re-import DSL into the existing app (app_id targets the update)
        with Session(db.engine, expire_on_commit=False) as session:
            dsl_service = AppDslService(session)
            result = dsl_service.import_app(
    ...
    @setup_required
    @enterprise_inner_api_only
    @inner_api_ns.doc("provisioning_delete_app")
    @inner_api_ns.doc(description="Delete an app")
    @inner_api_ns.doc(
        responses={
            204: "App deleted",
            404: "App not found",
...
def _get_system_account(workspace_id: str) -> Account:
    ...
    Used as the actor for update operations where the caller is a machine
    identity (no human email).  Falls back to the first active member.
    """
